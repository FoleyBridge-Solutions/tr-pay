<div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <flux:button
                        wire:click="openPaymentRequestModal"
                        variant="ghost"
                        class="w-full"
                        icon="envelope"
                    >
    ...
    <flux:modal name="send-payment-request" class="w-full max-w-lg">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Send Payment Request</flux:heading>
                <flux:text class="text-zinc-500 mt-1">
                    Send an email with a secure payment link to <span class="font-medium">{{ $client['client_name'] ?? '' }}</span>.
                </flux:text>
            </div>

            <form wire:submit.prevent="sendPaymentRequest" class="space-y-4">
                {{-- Email --}}
                <flux:field>
                    <flux:label>Recipient Email</flux:label>
                    @if(count($clientEmails) > 0)
                        <flux:select wire:model="paymentRequestEmail" placeholder="Select an email...">
                            @foreach($clientEmails as $email)
                                <flux:select.option value="{{ $email }}">{{ $email }}</flux:select.option>
                            @endforeach
                        </flux:select>
                    @else
                        <flux:input wire:model="paymentRequestEmail" type="email" placeholder="client@example.com" required />
                    @endif
                    <flux:error name="paymentRequestEmail" />
                </flux:field>

                {{-- Amount --}}
                <flux:field>
                    <flux:label>Amount</flux:label>
                    <flux:input wire:model="paymentRequestAmount" type="number" step="0.01" min="0.01" placeholder="0.00" required />
                    <flux:error name="paymentRequestAmount" />
                </flux:field>

                {{-- Invoice Selection --}}
                @if(count($openInvoices) > 0)
                    <div>
                        <flux:label class="mb-2">Select Invoices</flux:label>
                        <div class="max-h-48 overflow-y-auto">
                            <x-invoice-selection-table
                                :invoices="$openInvoices"
                                wire-model="paymentRequestInvoices"
                                :compact="true"
                            />
                        </div>
                    </div>
                ...
                <flux:field>
                    ...
                    <flux:textarea wire:model="paymentRequestMessage" rows="3" placeholder="Include a note to the client..." />
                    <flux:error name="paymentRequestMessage" />
                    <flux:description>This message will appear in the email and on the payment page.</flux:description>
                </flux:field>

                {{-- Info --}}
                <div class="bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg p-3 text-xs text-zinc-500 space-y-1">
                    <p>The link is single-use and expires in 30 days.</p>
                    <p>A receipt will be sent to the client after payment.</p>
                </div>

                {{-- Actions --}}
                <div class="flex justify-end gap-3 pt-2">
                    <flux:button type="button" wire:click="$dispatch('close-modal', { name: 'send-payment-request' })" variant="ghost">Cancel</flux:button>
                    <flux:button type="submit" variant="primary" icon="paper-airplane">
                        <span wire:loading.remove wire:target="sendPaymentRequest">Send Request</span>
                        <span wire:loading wire:target="sendPaymentRequest">Sending...</span>
                    </flux:button>
                </div>
            </form>
        </div>
