 ...
 */
 ...
 require_once '/var/www/itflow-ng/portal/check_login.php';
 
 require_once '/var/www/itflow-ng/portal/portal_functions.php';
...
if (isset($_POST['add_ticket_comment'])) {
    ...
    if (verifyContactTicketAccess($ticket_id, "Open")) {
        ...
        // Add the comment
        mysqli_query($mysqli, "INSERT INTO ticket_replies SET ticket_reply = '$comment', ticket_reply_type = 'Client', ticket_reply_by = $contact_id, ticket_reply_ticket_id = $ticket_id");
...
if (isset($_POST['add_ticket_feedback'])) {
    ...
    if (verifyContactTicketAccess($ticket_id, "Closed")) {
        ...
        // Add feedback
        mysqli_query($mysqli, "UPDATE tickets SET ticket_feedback = '$feedback' WHERE ticket_id = $ticket_id AND ticket_client_id = $client_id LIMIT 1");

        // Notify on bad feedback
        if ($feedback == "Bad") {
            mysqli_query($mysqli, "INSERT INTO notifications SET notification_type = 'Feedback', notification = '$contact_name rated ticket ID $ticket_id as bad', notification_client_id = $client_id");
        }
...
if (isset($_GET['close_ticket'])) {
    ...
    if (verifyContactTicketAccess($ticket_id, "Open")) {
        ...
        // Close ticket
        mysqli_query($mysqli, "UPDATE tickets SET ticket_status = 5, ticket_closed_at = NOW() WHERE ticket_id = $ticket_id AND ticket_client_id = $client_id");

        // Add reply
        mysqli_query($mysqli, "INSERT INTO ticket_replies SET ticket_reply = 'Ticket closed by $contact_name.', ticket_reply_type = 'Client', ticket_reply_by = $contact_id, ticket_reply_ticket_id = $ticket_id");

        //Logging
        mysqli_query($mysqli, "INSERT INTO logs SET log_type = 'Ticket', log_action = 'Closed', log_description = '$ticket_id Closed by client', log_ip = '$ip', log_user_agent = '$user_agent'");
...
if (isset($_POST['edit_profile'])) {
    ...
    if (!empty($new_password)) {
        ...
        // Logging
        mysqli_query($mysqli, "INSERT INTO logs SET log_type = 'Contact', log_action = 'Modify', log_description = 'Client contact $contact_name modified their profile/password.', log_ip = '$ip', log_user_agent = '$user_agent', log_client_id = $client_id");
    }
    header('Location: index.php');
    exit;
}
