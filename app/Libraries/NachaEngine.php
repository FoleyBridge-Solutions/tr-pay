...
class nacha_file
{
    ...
    private $settlement_is_savings = false;
    ...
    {
        // ACH OR74
        if (! $this->numeric_only($origin_id, 10)) {
            $this->errors[] = 'Invalid origin ID';
        }

        if (! $this->valid_aba($settlement_routing_number)) {
            $this->errors[] = 'Invalid settlement routing number';
        }

        if (! $this->alpha_numeric_only($file_modifier, 1)) {
            $this->errors[] = 'Invalid file modifier';
        }

        if (! $this->alphameric_only($originating_bank_name, 23)) {
            $this->errors[] = 'Invalid originating bank name';
        }

        if (! $this->alphameric_only($company_name, 23)) {
            $this->errors[] = 'Invalid company name';
        }
        ...
        $this->settlement_is_savings = $settlement_is_savings;
    ...
    private function create_batch($memo, $personal_payment = false, $service_class_code = '200')
    {
        ...
        if ($this->open_batch) {
            $this->close_batch($this->number_of_batches);
        } else {
            $this->open_batch = true;
        }
        ...
        $this->batches[$this->number_of_batches]['entries'] = [];
    ...
    {
        if (! $this->valid_aba($bank_routing_to)) {
            $this->errors[] = 'Invalid debit routing number for transaction: '.$internal_id;
        }
        if (! $this->numeric_only($bank_account_to, 17)) {
            $this->errors[] = 'Invalid debit account number for transaction: '.$internal_id;
        }
        if (! $this->alphameric_only($internal_id, 15)) {
            $this->errors[] = 'Invalid internal ID for transaction: '.$internal_id;
        }
        if (! $this->alphameric_only($name, 22)) {
            $this->errors[] = 'Invalid name for debit entry for transaction: '.$internal_id;
        }

        if (! $this->valid_decimal($amount)) {
            $this->errors[] = 'Invalid debit amount for transaction: '.$internal_id;
        }
        ...
        if ($create_new_batch) {
            $this->create_batch($memo, $personal_payment);
    ...
    {
        if (! $this->valid_aba($bank_routing_to)) {
            $this->errors[] = 'Invalid credit routing number for transaction: '.$internal_id;
        }
        if (! $this->numeric_only($bank_account_to, 17)) {
            $this->errors[] = 'Invalid credit account number for transaction: '.$internal_id;
        }
        if (! $this->alphameric_only($internal_id, 15)) {
            $this->errors[] = 'Invalid internal ID for transaction: '.$internal_id;
        }
        if (! $this->alphameric_only($name, 22)) {
            $this->errors[] = 'Invalid name for credit entry for transaction: '.$internal_id;
        }

        if (! $this->valid_decimal($amount)) {
            $this->errors[] = 'Invalid credit amount for transaction: '.$internal_id;
        }
    ...
    private function close_file()
    {
        ...
        // If balanced file, add consolidated offset entry before closing batches
        if ($this->balanced_file) {
            $this->add_consolidated_offset();
        }
    ...
    /**
     * Add a single consolidated offset entry to balance the file.
     * Called automatically by close_file() when balanced_file is true.
     * Creates offset credits for net debits, or offset debits for net credits.
     */
    private function add_consolidated_offset()
    {
        ...
        foreach ($this->batches as $batch) {
            $total_debits += $batch['debits'];
            $total_credits += $batch['credits'];
        }
        ...
        if ($this->open_batch) {
            $this->close_batch($this->number_of_batches);
            $this->open_batch = false;
        }

        // Create a new batch for the offset entry
        $this->create_batch('OFFSET', false); // CCD batch for offset

        $this->entry_count++;

        if ($net_amount > 0) {
            // Net debits - need a credit to settlement account
            // Transaction code: 22 checking credit, 32 savings credit
            $transaction_code = $this->settlement_is_savings ? '32' : '22';
            $this->batches[$this->number_of_batches]['credits'] += $net_amount;
        } else {
            // Net credits - need a debit from settlement account
            // Transaction code: 27 checking debit, 37 savings debit
            $transaction_code = $this->settlement_is_savings ? '37' : '27';
            $net_amount = abs($net_amount);
            $this->batches[$this->number_of_batches]['debits'] += $net_amount;
        }

        $this->batches[$this->number_of_batches]['entries'][$this->entry_count] = '6'.
            $transaction_code.
            $this->format_number($this->settlement_routing_number, 9).
            ''. // Omitted "check digit"
            $this->format_text($this->settlement_account_number, 17).
            $this->format_number($net_amount, 10). // Already in cents
            $this->format_text('OFFSET', 15).
        ...
        $this->batches[$this->number_of_batches]['hash'] += substr($this->settlement_routing_number, 0, 8);
     ...
     */
    public function valid_aba($aba_number)
    {
        $aba_length = strlen($aba_number);

        if (! $this->numeric_only($aba_number, 9) || $aba_length != 9) {
            return false;
        }
    ...
    {
        return substr(str_pad(str_replace(['.', ','], '', (string) $number), $number_of_spaces_to_insert, '0', STR_PAD_LEFT), ($number_of_spaces_to_insert) * -1);
    }
